...
# Data folder
datadir <- '/home/bniebuhr/Documents/00_academico/07_projetos/04_oncas_on_the_move/_Pardas_Tiete/data/'
...
meta.data <- readr::read_csv2('meta_data.csv')
meta.data
...
mov.data.puma.f1e2 <- readr::read_tsv('pardas_tiete_f1e2_Sucuri_Pora_2018_02_d20.csv', locale = locale(decimal_mark = ',')) %>% 
  mutate(UTC_Date = as.character(UTC_Date),
         UTC_Time = as.character(UTC_Time),
         timestamp = as.POSIXct(paste(UTC_Date, UTC_Time), format = '%d/%m/%Y %H:%M', tz = 'UTC'))
  
mov.data.puma.f1e2
mov.data.puma.f1e2 %>% print(width = Inf)
ncol(mov.data.puma.f1e2)
str(mov.data.puma.f1e2)

# F3 - Luna - collar not retrieved, in safety mode

# All other individuals - Pardas do Tiete and Legado das Aguas project
...
ch <- meta.data$name %>% iconv(to = 'ASCII//TRANSLIT') %>% 
  .[-c(1:2)] # extract two females loaded above
...
for(i in 1:length(files2read)) {
  ...
  # if(grepl('Mineiro|Piloto', files2read[i])) {
  #   ind.data[[i]] <- readr::read_csv(files2read[i]) %>% 
  #     mutate(UTC_Date = as.character(UTC_Date),
  #            UTC_Time = as.character(UTC_Time),
  #            timestamp = as.POSIXct(paste(UTC_Date, UTC_Time), format = '%m/%d/%Y %H:%M', tz = 'UTC'))
  # } else {
    ind.data[[i]] <- readr::read_csv(files2read[i]) %>% 
      mutate(UTC_Date = as.character(UTC_Date),
             UTC_Time = as.character(UTC_Time),
             timestamp = as.POSIXct(paste(UTC_Date, UTC_Time), format = '%Y-%m-%d %H:%M', tz = 'UTC'))
  # }
}
ind.data
ind.data[[6]] %>% print(width = Inf)
...
# Standardize column name hdop
for(i in 1:length(ind.data)) {
  if('HDOP' %in% colnames(ind.data[[i]])) {
    ind.data[[i]] <- ind.data[[i]] %>% 
      rename(hDOP = HDOP)
  }
}
...
mov.datasets.all <- mov.datasets %>% 
  dplyr::left_join(meta.data, by = 'ID') %>%
  dplyr::filter(timestamp >= release.date.utc, Latitude > -40) %>%
  dplyr::select(-c(Name_old, name, Sex, release.date.utc, last.date.utc, observation, 
            UTC_Date, UTC_Time, ends_with('.y'))) %>% 
  dplyr::rename(Tag_ID = Tag_ID.x, name = Name)
...
shp.complete <- shp %>% 
  sf::st_transform(crs = '+proj=aea +lat_1=-2 +lat_2=-22 +lat_0=-12 +lon_0=-54 +x_0=0 +y_0=0 +ellps=GRS80 +units=m +no_defs') %>% 
  sf::st_coordinates() %>% 
  cbind(shp.complete) %>%
...
for(i in studies) {
  to.save <- shp.complete %>% 
    dplyr::filter(study.name == i)
  
  setwd(outdir)
  sf::st_write(to.save, paste0('movement_data_', i, '_all_individuals_',
  ...
  to.save %>% dplyr::select(ID, name, timestamp) %>%
    sf::st_write(paste0('movement_data_', i, '_all_individuals_',
